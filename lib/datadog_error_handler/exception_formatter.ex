defmodule DatadogErrorHandler.ExceptionFormatter do
  @moduledoc """
  Provides exception formatting utilities.
  """

  @doc """
  Formats an error with given message and metadata.
  """
  ...
  defp crash_title(reason, stacktrace) do
  ...
  defp aggregation_key(title, {:stacktrace, stacktrace}) do
    String.slice(title, 0..40) <> (stacktrace |> List.first() |> inspect())
  end

  defp aggregation_key(title, {:meta, %{file: file, line: line}}) do
    String.slice(title, 0..40) <> file <> to_string(line)
  end

  defp aggregation_key(title, _) do
